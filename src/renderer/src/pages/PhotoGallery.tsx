...
//     try {
//       const apiUrl = new URL(`http://localhost:3000/api/albums/${albumId}/photos`)
...
//   const filteredPhotos = photos.filter((photo) => !selectedDate || photo.date === selectedDate)

//   const isInCart = selectedPhoto
//     ? cart.some(
//         (item) =>
...
//                     <p className="text-gray-200 text-sm">{format(new Date(photo.date), 'PP')}</p>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//                         <span
//                           className={
//                             selectedFormat === format.value ? 'text-white' : 'text-blue-600'
//                           }
//                         >
...
//                             <p className="text-xs font-medium text-white px-1">{filter.name}</p>
//                           </div>
//                         </div>
//                       </button>
//                     ))}
...
//                         {isLoading ? 'Chargement...' : isInCart ? 'Dans le panier' : t('addToCart')}
//                       </span>
//                     </button>
//                     <button
...
export const PhotoGallery: React.FC = () => {
  ...
  const fetchPhotos = async (pageNumber: number) => {
    ...
    try {
      const apiUrl = new URL(`http://localhost:3000/api/albums/${albumId}/photos`);
      apiUrl.searchParams.append('page', pageNumber.toString());
      apiUrl.searchParams.append('limit', '50'); // Fetch 50 images per page
      ...
      if (!Array.isArray(responseData)) {
        throw new Error('La structure des données de l\'API est invalide');
      }
  
      const formattedPhotos: Photo[] = responseData
        .filter((photo: APIPhoto) => !photo.is_deleted && photo.status === 'AVAILABLE')
        .map((photo: APIPhoto) => ({
          id: photo._id,
          albumId: photo.album_id._id,
          title: photo.filename,
      ...
      setPhotos((prev) => (pageNumber === 1 ? formattedPhotos : [...prev, ...formattedPhotos]));
      setTotalPages(Math.ceil(responseData.length / 50)); // Calculate total pages based on the number of photos
    } catch (err) {
      console.error('Erreur lors du chargement des photos:', err);
      setError(
        err instanceof Error ? err.message : 'Une erreur est survenue lors du chargement des photos'
          ...
          <div className="space-y-4">
            {filteredPhotos.map((photo) => (
              <div
                key={photo.id}
                onClick={() => setSelectedPhoto(photo)}
                className="cursor-pointer flex items-center gap-4 p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow"
              >
                <img
                  src={photo.url}
                  alt={photo.title}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{format(new Date(photo.date), 'PP')}</p>
                </div>
              </div>
            ))}
          </div>
